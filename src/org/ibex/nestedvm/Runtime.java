...
public abstract class Runtime implements UsermodeConstants, Registers, Cloneable {
  ...
  /** Writable main memory pages. If the page is writable writePages[x] == readPages[x]; if not writePages[x] == null. */
  ...
  /**
   * The size of the _user_info block
   * 
   * @return the size
   */
  ...
  /**
   * Table containing all open file descriptors. (Entries are null if the fd is
   * not in use
   */
  ...
  /**
   * Set the call to java
   * 
   * @param callJavaCB call routine to use
   */
  ...
  /**
   * Subclasses should actually execute program in this method. They should
   * continue executing until state != RUNNING. Only syscall() can modify state.
  ...
  /**
   * Subclasses should return the address of the symbol <i>symbol</i> or -1 it
   * it doesn't exits in this method This method is only required if the call()
   * function is used
   *
  ...
  protected Object clone() throws CloneNotSupportedException {
    ...
    for (int i = 0; i < totalPages; i++) {
      ...
      if (writePages[i] == null) {
        ...
        r.readPages[i] = r.writePages[i] = writePages[i].clone();
      }
    }
    return r;
  }
  ...
  /**
   * Copy everything from <i>src</i> to <i>addr</i> initializing uninitialized
   * pages if required. Newly initalized pages will be marked read-only if
   * <i>ro</i> is set
   *
   ...
   * @param buf buffer 
   * @param count the length to copy
   * @throws org.ibex.nestedvm.Runtime.ReadFaultException In case od read problem
   */
  ...
  /**
   * Copies <i>length</i> bytes OUT OF the java array <i>a</i> into the
   * processes memory space at <i>addr</i>
   * 
   * @param buf the buffer 
   * @param addr the starting address
   * @param count the length to copy
   * @throws org.ibex.nestedvm.Runtime.FaultException In case of error
   */
  @SuppressWarnings("fallthrough")
  public final void copyout(byte[] buf, int addr, int count) throws FaultException {
    ...
    if ((addr & 3) != 0) {
      ...
      switch (addr & 3) {
        ...
        case 3:
          word = (word & 0xffffff00) | ((buf[x++] & 0xff));
          if (--count == 0) {
            break;
          }
      }
  ...
  /**
   * Set a region of memory with the given value
   * 
   * @param addr the starting address
   * @param ch the value to set
   * @param count the length of area
   * @throws org.ibex.nestedvm.Runtime.FaultException In case of error
   */
  @SuppressWarnings("fallthrough")
   ...
   * 
   * @param addr the address where to read
   * @return the read value 
   * @throws org.ibex.nestedvm.Runtime.ReadFaultException In case of error
   */
  ...
  /**
   * Read a word from the unsafe processes memory at <i>addr</i>
   * 
   * @param addr the address where to read
   * @return the read value 
   * @throws org.ibex.nestedvm.Runtime.ReadFaultException In case of error
   ...
   * @throws org.ibex.nestedvm.Runtime.WriteFaultException In case of error
   */
  public final void memWrite(int addr, int value) throws WriteFaultException {
   ...
   /**
   * Writes a word to the unsafe processes memory at <i>addr</i>
   * 
   * @param addr the address where to write
   * @param value the value to write
   * @throws org.ibex.nestedvm.Runtime.WriteFaultException In case of error
...
//    private final int[] initPage(int page) { return initPage(page,false); }
   ...
   * set the page will be read-only
   * 
   ...
   * 
   * @param strings the string to add
   ...
   * Create enviroment
   * 
   * @param extra the input enviroment
   ...
   * The user_info table is a chunk of memory in the program's memory defined by
   * the symbol "user_info". The compiler/interpreter automatically determine
   * the size and location of the user_info table from the ELF symbol table.
   * <code>setUserInfo</code> and <code>getUserInfo</code> are used to modify
   * the words in the user_info
   * table.
   * 
   ...
   * @return the user info at given index
  ...
  /**
   * Executes the process until the PAUSE syscall is invoked or the process
   * exits. Returns true if the process exited.
   * 
  ...
  /**
   * Concatenate a string in an array
   * 
   * @param argv0 the string to concatenate
   * @param rest the initial array
  ...
  /**
   * Call the running job with the passed arguments
   * 
   * @param argv0 the argument 0
   * @param rest the rest of argument
   * @return the result code
   */
  ...
  /**
   * Runs the process until it exits and returns the exit status. If the process
   * executes the PAUSE syscall execution will be paused for 500ms and a warning
   * will be displayed
   * 
   * @param args the arguments
   * @param env the enviroments
   * @return the result code
   */
  ...
  public final void start(String[] args, String[] environ) {
    ...
    if (args == null) {
      args = new String[]{getClass().getName()};
    }
   ...
   * 
   * @param sym the symbol of function
   * @param args the arguments of function
   * @return the result code
   * @throws org.ibex.nestedvm.Runtime.CallException In case of calling error
   ...
   * Calls a function in the process with the given arguments
   * 
   * @param sym sym the symbol of function
   * @param args the arguments of function
   * @return the result code
   * @throws org.ibex.nestedvm.Runtime.CallException In case of calling error
   */
  ...
  /**
   * Executes the code at <i>addr</i> in the process setting A0-A3 and S0-S3 to
   * the given arguments and returns the contents of V1 when the the pause
   * syscall is invoked
   * 
   * @param addr the address with the code
   * @param a0 argument
   * @param rest the enviroment
   * @return the result code
   * @throws org.ibex.nestedvm.Runtime.CallException In case of calling error
   */
  ...
  /**
   * Allocated an entry in the FileDescriptor table for <i>fd</i> and returns
   * the number. Returns -1 if the table is full. This can be used by subclasses
   * to use custom file descriptors
   * 
  ...
  /**
   * Hooks for subclasses before the process closes an FD
   */
  void _preCloseFD(FD fd) {
  }

  /**
   * Hooks for subclasses after the process closes an FD
   */
  void _postCloseFD(FD fd) {
  }

  /**
   * Closes file descriptor <i>fdn</i> and removes it from the file descriptor
   * table
   * 
   * @param fdn the file descripton
  ...
  private int sys_open(int addr, int flags, int mode) throws ErrnoException, FaultException {
    ...
    if (name.length() == 1024 && getClass().getName().equals("tests.TeX")) {
      name = name.trim();
    }
  ...
  /**
   * The syscall dispatcher. The should be called by subclasses when the syscall
   * instruction is invoked.
   * <i>syscall</i> should be the contents of V0 and <i>a</i>, <i>b</i>,
   * <i>c</i>, and <i>d</i> should be the contenst of A0, A1, A2, and A3. The
   * call MAY change the state
   *
   * @see Runtime#state state
   */
  ...
  public static abstract class FD {
    ...
    /**
     * Seek in the filedescriptor. Whence is SEEK_SET, SEEK_CUR, or SEEK_END.
     * Should return -1 on error or the new position.
     */
    ...
    /**
     * Return a Seekable object representing this file descriptor (can be read
     * only) This is required for exec()
     */
    Seekable seekable() {
      return null;
    }
  ...
  public static class InputOutputStreamFD extends FD {
    ...
    public int read(byte[] a, int off, int length) throws ErrnoException {
      if (is == null) {
        return super.read(a, off, length);
      }
      try {
  ...
  static class HostFStat extends FStat {
    ...
    public HostFStat(File f, Seekable.File sf) {
      this(f, sf, false);
    }
    ...
    public int mode() {
      ...
      if (canread && (executable || f.isDirectory())) {
        mode |= 0111;
      }
      if (canread) {
        mode |= 0444;
